...
export class UserService {
  ...
  async getAll() {
    ...
    if (users) {
      ...
      return new HttpException('No users were found in the database',HttpStatus.NOT_FOUND);
    }
    } catch (error) {
      return error;
    }
  ...
  async update(id: any, updateUserDto: UpdateUserDto) {
    try {
      ...
      if (response.affected) {
        return new HttpException("User with id " + id + " updated",HttpStatus.OK);
      } else {
        return new HttpException("User id " + id + " not found in the database",HttpStatus.NOT_FOUND);
      }
     } catch (error) {
      return error;
    }
  ...
  async remove(id: any) {
    try {
      ...
      if (response.affected) {
        return new HttpException("User with id " + id + " removed successfully",HttpStatus.OK);
      } else {
        return new HttpException("User id " + id + " not found in the database",HttpStatus.NOT_FOUND);
      }
    } catch (error) {
